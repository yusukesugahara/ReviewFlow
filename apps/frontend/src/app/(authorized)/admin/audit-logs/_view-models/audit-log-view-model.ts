import { readMetadata, shortId, textValue, valueList } from "../_utils/audit-log-metadata";
import type { AdminAuditLogsViewProps } from "../types";

type AuditRow = AdminAuditLogsViewProps["rows"][number];
...
export function describeTargetLabel(row: AuditRow, metadata = readMetadata(row.metadataJson)): string {
...
export function buildAuditSummaryCounts(rows: EnrichedAuditRow[]): AuditLogSummaryCounts {
  return {
    applicationEvents: rows.filter((item) => item.row.targetType === "application").length,
    identityEvents: rows.filter((item) => ["invitation", "user"].includes(item.row.targetType))
      .length,
    spaceEvents: rows.filter((item) => ["group_member", "space"].includes(item.row.targetType))
      .length,
    total: rows.length,
  };
}
...
export function buildAuditLogsHref({
  ...
  query,
  targetType,
}: Pick<AdminAuditLogsViewProps, "createdFrom" | "createdTo" | "query" | "targetType">): string {
...
function describeTargetDetail(row: AuditRow): string | null {
  ...
  if (row.targetId && row.targetId !== row.applicationId && row.targetId !== row.targetUserId) {
    parts.push(`対象ID: ${shortId(row.targetId)}`);
  }
  return parts.filter(Boolean).join(" / ");
}

function describeChanges(row: AuditRow, metadata: Record<string, unknown>): string[] {
  ...
  if (row.stepOrderFrom != null || row.stepOrderTo != null) {
    changes.push(`ステップ: ${formatNumber(row.stepOrderFrom)} -> ${formatNumber(row.stepOrderTo)}`);
  }
  ...
  if (typeof metadata.isActiveFrom === "boolean" || typeof metadata.isActiveTo === "boolean") {
    changes.push(
...
function buildDetailItems(row: AuditRow, metadata: Record<string, unknown>): AuditLogDisplayEntry[] {
...
function addItem(items: AuditLogDisplayEntry[], label: string, value: unknown): void {
...
function addListItem(items: AuditLogDisplayEntry[], label: string, value: unknown): void {
...
function hasAuditLogFilters({
  ...
  query,
  targetType,
}: Pick<AdminAuditLogsViewProps, "createdFrom" | "createdTo" | "query" | "targetType">): boolean {
  return (
    query.length > 0 ||
    targetType !== "all" ||
...
function describeAuditLogList({
  ...
  const targetLabel = TARGET_FILTER_LABELS[targetType as AuditLogTargetTypeFilter] ?? targetType;
  ...
  return query ? `「${query}」に一致する最新200件` : "最新200件を新しい順に表示しています";
}

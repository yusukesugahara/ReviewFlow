...
- API 契約を変える場合は GraphQL / Relay operation と共有 DTO 型を更新する。REST 契約を変える場合は Swagger / OpenAPI 参照スキーマも更新する
...
- API 通信では Relay operation helper と共有 DTO 型を優先する
...
- tenant / space 分離、backend authorization、状態遷移管理、Relay / GraphQL 連携を評価軸として含める

...
## ログイン中アカウント管理
- ログイン中ユーザーは `/account` で名前、メールアドレス、ロール、テナントID、パスワード設定状態を確認できる。
- プロフィール編集は `PATCH /auth/me/profile` で名前を更新し、新しい JWT を発行する。
- パスワード変更は `PATCH /auth/me/password` で現在のパスワードを検証してから更新し、新しい JWT を発行する。
- メールアドレス変更は即時更新せず、確認メールを送信してから確定する。

## メールアドレス変更
- ログイン中ユーザーが新しいメールアドレスを送信すると、即時には `users.email` を更新しない。
- `email_change_tokens` に変更リクエストを保存し、新しいメールアドレス宛に確認 URL を送信する。
- 確認 URL から変更確定を実行した時点で `users.email` を更新し、監査ログに変更前後のメールアドレスを記録する。
- メールアドレス確定後は既存 JWT のメールアドレスと DB の現在値が一致しなくなるため、再ログインを前提にする。
